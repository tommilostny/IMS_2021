...
#define __SIMLIB__        0x0308 // version of SIMLIB (numerical)
#define SIMLIB_VERSION    "3.08" // version of SIMLIB (string)
...
# error  "Use C++11 (or newer) compiler, please."
#endif
...
class SimObject {
 public:
  enum _Flags {         //!< internal flags (bitmasks: 1bit set)
        _CLEAR_ALL_FLAGS = 0,
        _ALLOCATED_FLAG  = 1<<0,        // object is on heap
        _EVAL_FLAG       = 1<<1,        // currently evaluated (aBlock)
        _HAS_NAME_FLAG   = 1<<2         // object has name (in dictionary)
  };
 protected:
  unsigned _flags;      //!< bool flags for internal use (TODO bitfield?)
 public:
  bool TestAndSetFlag(bool new_value, unsigned n); // internal use only
  SimObject();
  virtual ~SimObject();
...
// TODO: FIXME inconsistent name:
  bool isAllocated() const { return (_flags & _ALLOCATED_FLAG)!=0; }
 ...
 private:
  SimObject(const SimObject &) = delete;                //!< disable copy operation
  SimObject &operator= (const SimObject &) = delete;    //!< disable assign operation
};
...
//! struct EventNotice is  private to calendar implementation
struct EventNotice;     // we use only pointer to this class here
...
//    virtual void Into(Queue *q);         // insert itself into queue
    virtual void Out() override;        //!< remove entity from queue

  private:
    // Simulation control algorithm interface:
    virtual void _Run() noexcept = 0;   //!< run the Behavior() function
    // friends:
    friend void SIMLIB_DoActions();     // internal function - event dispatcher

    // Calendar interface:
    friend struct EventNotice;          // internal calendar class sets _Ev
...
#ifdef I_REALLY_KNOW_HOW_TO_USE_WAITUNTIL
...
# define WaitUntil(condition)  while(_WaitUntil(condition)) /*empty body*/;
  ...
  virtual void Terminate() override;             //!< kill process
...
class aStat : public SimObject {
 ...
 public:
  ...
  virtual void Output() const override;        //!< Print all values to output file
};
#endif
...
class TStat : public SimObject {
 protected:
  ...
  friend class Queue;
 public:
  explicit TStat(double initval=0.0);
  explicit TStat(const char *name, double initval=0.0);
  ~TStat();
...
class List : public Link { // circular list of Link items
 ...
 public:
    List();
    explicit List(const char *_name);
    ~List();
    virtual void Output() const override;          //!< print object to default output
    // should be renamed:
...
//TODO:remove
  ...
  public:
    ...
    Entity *front()    { return static_cast<Entity*>(List::front()); }
    Entity *back()     { return static_cast<Entity*>(List::back()); }
...
class Histogram : public SimObject {
 ...
 public:
  ...
  ~Histogram();
  virtual void Output() const override;         //!< print to default output
...
class Facility : public SimObject {
  unsigned char _Qflag;         //!< true if facility is owner of input queue
 protected:
 ...
 public:
  Facility();                   // for arrays only, TODO: FacilityArray
  explicit Facility(const char *_name);
  explicit Facility(Queue *_queue1);
  ...
  bool Busy() const { return in!=nullptr; }     //!< in service
  Entity * In() const { return in; }            //!< current entity or nullptr
  ...
  virtual void Clear();                          //!< initialize
 protected:
  virtual void QueueIn2(Entity *e);              // go into Q2
};
...
class Store : public SimObject {
  unsigned char _Qflag;         //!< true if store is owner of input queue
 protected:
  unsigned long capacity;       //!< Capacity of store
  unsigned long used;           //!< Currently used capacity
  Queue *Q;                     //!< input queue
  TStat tstat;                  //!< usage statistics
 public:
  Store();
  explicit Store(unsigned long _capacity);
...
//! \ingroup simlib
/// blocks/SimObject have not copy semantics (TODO: add move semantics)
...
class Constant : public aContiBlock {
 ...
 public:
  ...
  virtual double Value ()  override      { return value; }
};
...
class Variable : public aContiBlock {
  double value;
 public:
  explicit Variable(double x=0) : value(x) {}
...
class Parameter : public aContiBlock {
  double value;
 public:
  explicit Parameter(double x) : value(x) {}
...
// (and in dynamical structure models at simulation time, too).
...
class Input {
 ...
 public:
  ...
  // ## we use implicit conversions to simplify block expressions
  ...
  //! WARNING: using C++ variable in block expression creates Constant block
...
struct Expression : public aContiBlock1 {
  explicit Expression(Input i) : aContiBlock1(i) {}
  double Value() override;       //!< Evaluate expression and return the value
};
...
protected:
  ...
  class Memory {
    ...
    public:
      explicit Memory(std::list<Memory*> * PtrList=PtrMList);  // create empty memory
      virtual ~Memory();
...
public:
  explicit IntegrationMethod(const char* name);  // register the method with name
  ...
  static bool IsConditionFlag(void);    // any changes of condition vector?
...
private:
  ...
  bool StartMode;
public:
  explicit SingleStepMethod(const char* name):
    IntegrationMethod(name),
    StartMode(false) { /* NOTHING */ }
    virtual bool IsSingleStep(void) override { return true; }
...
public:
  ...
  virtual bool IsSingleStep(void) override { return false; }
  ...
  virtual bool PrepareStep(void) override;  // prepare the object for the step
  virtual void TurnOff(void) override;  // turn off method & its slave
...
//## repair
  ...
  public:
    // create empty memory
    explicit StatusMemory(std::list<Memory*>* PtrList=PtrStatusMList) : Memory(PtrList) { }
    // free dynamic data
...
public:
  explicit StatusMethod(const char* name);  // initailization
  virtual ~StatusMethod() { } // free dynamic datas
  virtual void TurnOff(void) override;  // turn off integration method
  virtual bool PrepareStep(void) override;  // prepare integration step
...
class Integrator : public aContiBlock {   // integrator
 ...
 public:
  ...
  // we use implicit conversions in block expressions
...
class aCondition : public aBlock {
  static aCondition *First;            // list of all state conditions
...
class Condition : public aCondition {  // state condition
 ...
 protected:
  virtual bool Test() override;         // test function (input >= 0.0)
  bool Up()     { return ccl<cc; }      // change: FALSE->TRUE
  bool Down()   { return ccl>cc; }      // change: TRUE->FALSE
  bool Change() { return ccl!=cc; }     // both changes
 public:
  explicit Condition(Input i);
  ~Condition();
  bool Value() { return cc; }           //!< boolean state of condition
...
//TODO  Rline(Input in, int num, double table[][2]);
  ~Rline();
  virtual void _Eval() override;           // evaluation with loop check
  virtual double Value() override;         // returned value
  //virtual const char *Name() const;
...
// (not checked, needs improvements)
//! \ingroup simlib
...
////////////////////////////////////////////////////////////////////////////
// Naming Subsystem --- SimObject names (text identifiers for printing)
...
class Semaphore : public SimObject {
 ...
 public:
  ...
  explicit Semaphore(const char *_name);        // with associated name
  virtual ~Semaphore();
...
class Barrier : public SimObject {
 ...
 public:
  ...
  void Clear();                         //!< initialization
  virtual void Output() const override;          //!< print status
};
...
// TODO: add real time of execution ?
//! \ingroup simlib
